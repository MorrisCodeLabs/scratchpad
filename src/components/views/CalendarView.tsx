...
export function CalendarView() {
  ...
  return (
    <div className="mx-auto flex h-full max-w-5xl gap-8 overflow-y-auto px-8 py-8">
      <div className="flex-1">
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-2xl font-bold text-ink">{format(month, "MMMM yyyy")}</h1>
          <div className="flex items-center gap-1">
            <button onClick={() => setMonth((m) => subMonths(m, 1))} className="rounded-md p-1.5 text-muted hover:bg-surface-2">
              <ChevronLeft size={16} />
            </button>
            <button onClick={() => setMonth(new Date())} className="rounded-md px-2 py-1 text-xs text-muted hover:bg-surface-2">
              Today
            </button>
            <button onClick={() => setMonth((m) => addMonths(m, 1))} className="rounded-md p-1.5 text-muted hover:bg-surface-2">
          ...
          {days.map((day) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex h-20 flex-col items-start gap-1 rounded-md border border-transparent p-2 text-left",
      ...
      <div className="w-72 shrink-0 border-l border-line pl-6">
            ...
            {selectedNotes.map((note) => (
              <li key={note.id}>
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm text-muted hover:bg-surface-2 hover:text-ink"
                >
